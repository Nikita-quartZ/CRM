document.addEventListener("DOMContentLoaded", () => {
  ...
  function createTableUsers(users) {
    ...
    for (const user in users) {
      ...
      colomn.innerHTML = `${info.name} ${info.surname} ${info.lastName}`;
      row.appendChild(colomn);
      colomn = document.createElement("td");
  ...
  function changeContact(contacts) {
    ...
    mainBlock.classList.add("contacts-block");
    ...
    for (const contact of contacts) {
      if (ind === 4) {
        ...
        btn.addEventListener("click", () => {
          ...
          for (const contact of contacts) {
            const btn = document.createElement("button");
            btn.classList.add("contact-btns");
            if (contact.type == "Телефон") {
              btn.innerHTML = svgPhone;
            } else if (contact.type == "Email") {
              btn.innerHTML = svgMail;
            } else if (contact.type == "Facebook") {
              btn.innerHTML = svgFb;
            } else if (contact.type == "Vk") {
              btn.innerHTML = svgVk;
            } else if (contact.type == "profile") {
              btn.innerHTML = svgProfile;
            }
            tippy(btn, {
  ...
  function changeFormatDate(date) {
    ...
    return `${day}.${month}.${date.getFullYear()} <span class="catalog__time">${hour}:${minute}</span>`;
  }
  ...
  function createUser(name, surname, lastName, contacts) {
    let user;
    if (contacts != []) {
      user = {
        name: name,
  ...
  document
    ...
    .addEventListener("click", () => {
      createUser("Василий", "Пупкин", "Васильевич", [
        {
          type: "Телефон",
          value: "+71234567890",
        },
  ...
  function getUser(id) {
    return fetchJson(`/api/catalog${id}`);
  }
